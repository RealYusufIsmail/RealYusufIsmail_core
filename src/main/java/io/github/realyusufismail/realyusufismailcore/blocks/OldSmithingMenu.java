...
import io.github.realyusufismail.realyusufismailcore.blocks.recipe.OldSmithingRecipe;
...
public class OldSmithingMenu extends ItemCombinerMenu {
    ...
    @Nullable
    private OldSmithingRecipe selectedRecipe;
    private final List<OldSmithingRecipe> recipes;

    public OldSmithingMenu(int p_267173_, Inventory p_267175_) {
        this(p_267173_, p_267175_, ContainerLevelAccess.NULL);
    }

    public OldSmithingMenu(int p_266937_, Inventory p_267213_, ContainerLevelAccess p_266723_) {
        super(MenuType.LEGACY_SMITHING, p_266937_, p_267213_, p_266723_);
        this.level = p_267213_.player.level;
        this.recipes = this.level.getRecipeManager().<Container, SmithingRecipe>getAllRecipesFor(RecipeType.SMITHING).stream().filter((p_266879_) -> p_266879_ instanceof OldSmithingRecipe).map((p_266816_) -> (OldSmithingRecipe)p_266816_).toList();
    }
    ...
    public void createResult() {
        List<OldSmithingRecipe> list = this.level.getRecipeManager().getRecipesFor(RecipeType.SMITHING, this.inputSlots, this.level).stream().filter((p_267116_) -> {
            return p_267116_ instanceof OldSmithingRecipe;
        }).map((p_266971_) -> {
            return (OldSmithingRecipe)p_266971_;
        }).toList();
        if (list.isEmpty()) {
            this.resultSlots.setItem(0, ItemStack.EMPTY);
        } else {
            OldSmithingRecipe oldSmithingRecipe = list.get(0);
            ItemStack itemstack = oldSmithingRecipe.assemble(this.inputSlots, this.level.registryAccess());
